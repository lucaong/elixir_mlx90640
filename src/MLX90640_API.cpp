...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    // This gets the chip unstuck in cases in which otherwise data would never
    // come (status register bit 3 always stays at 0)
    /*
...
int MLX90640_SetSubPageMode(uint8_t slaveAddr, uint8_t deviceMode)
{
    uint16_t controlRegister1;
    int value;
    int error;
    
    value = (deviceMode & 0x01);
    
    error = MLX90640_I2CRead(slaveAddr, 0x800D, 1, &controlRegister1);
    if(error == 0)
    {
        value = (controlRegister1 & 0b11111111111111110) | value;
        error = MLX90640_I2CWrite(slaveAddr, 0x800D, value);
    }    
...
int MLX90640_SetDataHold(uint8_t slaveAddr, uint8_t dataHold)
{
    uint16_t controlRegister1;
    int value;
    int error;
    
    value = (dataHold & 0x01)<<2;
...
int MLX90640_SetControlRegister(uint8_t slaveAddr, uint16_t value)
{
    uint16_t controlRegister1;
    int error;
    ...
    if(error == 0)
    {
        value = (controlRegister1 & 0b1110000000000000) | (value & 0b0001111111111111);
        error = MLX90640_I2CWrite(slaveAddr, 0x800D, value);
    }    
